...
template <typename T>
QList<T> listFrom(const QJsonDocument& doc)
{
    QList<T> out;
...
void ApiClient::restoreSession(const QString& refreshToken, const ApiUser& user)
{
...
void ApiClient::applySession(const QJsonObject& tokenPair)
{
    ...
    m_user = ApiUser::fromJson(tokenPair.value(QStringLiteral("user")).toObject());
...
void ApiClient::send(const QByteArray& method, const QString& path,
                     ...
                     RawHandler onOk, ErrorHandler onError, bool mayRetry)
{
    if (!hasBaseUrl()) {
        report(onError,
...
void ApiClient::dispatch(const PendingRequest& request)
{
    ...
    if (!m_accessToken.isEmpty())
        netRequest.setRawHeader("Authorization", "Bearer " + m_accessToken.toUtf8());

    const QByteArray payload =
        request.hasBody ? QJsonDocument(request.body).toJson(QJsonDocument::Compact)
                        : QByteArray{};
...
{
    send("GET", QStringLiteral("/health"), {}, false,
                                ...
                                Handler<ApiUser> onOk, ErrorHandler onError)
{
    ...
    send("POST", QStringLiteral("/auth/register"), body, true,
                      ...
                      Handler<ApiUser> onOk, ErrorHandler onError)
{
    ...
    send("POST", QStringLiteral("/auth/login"), body, true,
         [this, onOk](const QJsonDocument& doc) {
             applySession(objectFrom(doc));
             if (onOk)
                 onOk(m_user);
...
void ApiClient::refreshSession(VoidHandler onOk, ErrorHandler onError)
{
    ...
    send("POST", QStringLiteral("/auth/refresh"), body, true,
         [this, onOk](const QJsonDocument& doc) {
             m_refreshing = false;
             applySession(objectFrom(doc));
             if (onOk)
...
void ApiClient::listGroups(Handler<QList<ApiGroup>> onOk, ErrorHandler onError)
{
    send("GET", QStringLiteral("/groups"), {}, false,
         [onOk](const QJsonDocument& doc) {
             if (onOk)
...
{
    send("POST", QStringLiteral("/groups"),
...
{
    send("PATCH", QStringLiteral("/groups/%1").arg(groupId),
         QJsonObject{{QStringLiteral("name"), name}}, true,
         [onOk](const QJsonDocument& doc) {
             if (onOk)
                 onOk(ApiGroup::fromJson(objectFrom(doc)));
...
void ApiClient::deleteGroup(int groupId, VoidHandler onOk, ErrorHandler onError)
{
    send("DELETE", QStringLiteral("/groups/%1").arg(groupId), {}, false,
...
void ApiClient::listMembers(int groupId, Handler<QList<ApiMember>> onOk,
                            ErrorHandler onError)
{
    send("GET", QStringLiteral("/groups/%1/members").arg(groupId), {}, false,
         [onOk](const QJsonDocument& doc) {
             if (onOk)
...
{
    send("POST", QStringLiteral("/groups/%1/members").arg(groupId),
...
void ApiClient::listFiles(int groupId, Handler<QList<ApiFile>> onOk,
                          ErrorHandler onError)
{
    send("GET", QStringLiteral("/groups/%1/files").arg(groupId), {}, false,
         [onOk](const QJsonDocument& doc) {
             if (onOk)
                             ...
                             ErrorHandler onError)
{
    ...
    send("POST", QStringLiteral("/groups/%1/files").arg(groupId), body, true,
         [onOk](const QJsonDocument& doc) {
             if (onOk)
...
void ApiClient::removeFile(int groupId, int fileId, VoidHandler onOk,
                           ErrorHandler onError)
{
    send("DELETE", QStringLiteral("/groups/%1/files/%2").arg(groupId).arg(fileId),
         {}, false,
         [onOk](const QJsonDocument&) {
             if (onOk)
                 onOk();
...
{
    send("GET", QStringLiteral("/tags"), {}, false,
         [onOk](const QJsonDocument& doc) {
             if (onOk)
...
void ApiClient::listGroupTags(int groupId, Handler<QList<ApiTag>> onOk,
                              ErrorHandler onError)
{
    send("GET", QStringLiteral("/groups/%1/tags").arg(groupId), {}, false,
         [onOk](const QJsonDocument& doc) {
             if (onOk)
                 onOk(listFrom<ApiTag>(doc));
         },
...
void ApiClient::createTag(int groupId, const QString& name, Handler<ApiTag> onOk,
                          ErrorHandler onError)
{
    send("POST", QStringLiteral("/groups/%1/tags").arg(groupId),
         QJsonObject{{QStringLiteral("name"), name}}, true,
         [onOk](const QJsonDocument& doc) {
             if (onOk)
...
{
    send("PATCH", QStringLiteral("/groups/%1/tags/%2").arg(groupId).arg(tagId),
         QJsonObject{{QStringLiteral("name"), name}}, true,
         [onOk](const QJsonDocument& doc) {
             if (onOk)
                 onOk(ApiTag::fromJson(objectFrom(doc)));
...
void ApiClient::deleteTag(int groupId, int tagId, VoidHandler onOk,
                          ErrorHandler onError)
{
    send("DELETE", QStringLiteral("/groups/%1/tags/%2").arg(groupId).arg(tagId),
         {}, false,
         [onOk](const QJsonDocument&) {
             if (onOk)
                 onOk();
...
void ApiClient::listFileTags(int groupId, int fileId, Handler<QList<ApiTag>> onOk,
                             ErrorHandler onError)
{
    send("GET",
                           ...
                           Handler<ApiNote> onOk, ErrorHandler onError)
{
    ...
    send("PATCH", QStringLiteral("/notes/%1").arg(noteId), payload, true,
         [onOk](const QJsonDocument& doc) {
             if (onOk)
                 onOk(ApiNote::fromJson(objectFrom(doc)));
         },
...
{
    send("DELETE", QStringLiteral("/notes/%1").arg(noteId), {}, false,
         [onOk](const QJsonDocument&) {
             if (onOk)
                 onOk();
         },
...
void ApiClient::syncStatus(int groupId, Handler<ApiSyncStatus> onOk,
                           ErrorHandler onError)
{
    send("GET", QStringLiteral("/groups/%1/sync-status").arg(groupId), {}, false,
         [onOk](const QJsonDocument& doc) {
             if (onOk)
                           ...
                           Handler<ApiUploadResult> onOk, ErrorHandler onError)
{
    ...
    if (!file->open(QIODevice::ReadOnly)) {
        ...
        delete file;
        report(onError,
               ApiError::network(QStringLiteral("Could not read %1.\n\n%2")
